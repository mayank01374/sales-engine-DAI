...
def active_config(db: Session) -> models.ScoringConfig:
    ...
    if not cfg:
        cfg = models.ScoringConfig(name="Default", is_active=True)
...
def seed(db: Session):
    ...
    if not db.query(models.SavedView).first():
        defaults=[
